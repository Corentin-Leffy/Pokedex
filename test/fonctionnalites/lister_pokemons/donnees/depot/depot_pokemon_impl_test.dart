...
void main() {
  ...
  int id = 1;
  ModelePokemon pokemon = ModelePokemon(
      id: id,
      ordre: 1,
      baseExperience: 64,
      nom: "Bulbizarre",
  ...
  group("Avec une connexion internet", () {
    setUp(() {
      when(informationsReseauInternet.aInternet)
  ...
  group("Sans connexion internet", () {
    ...
    void verifieInteractionsAvecSourceLocaleSeulement() {
      verify(sourceLocalePokemons.recupereDernierPokemon());
      verifyZeroInteractions(sourceDistantePokemons);
    }

    test("Récupére le dernier pokémon dans la source de données locale",
        () async {
      when(sourceLocalePokemons.recupereDernierPokemon())
          .thenAnswer((_) async => pokemon);
